...
// ver1.2
     ...
     * @return ¬озвращает строку:
            ...
            'width' - если изменение размера требуетс€ пропорционально по ширине окна
            'height' - если изменение размера требуетс€ пропорционально по высоте окна
     */
   ...
   function get_image($id, $width = 0, $height = 0)
    ...
    {
        $rows = db_get_list_by_query('SELECT * FROM object_images WHERE obj_type = "' . strtok($obj_type, " ") .
                                    '" AND obj_id = ' . (int)$obj_id . ' ORDER by `order` ' . strtok($order, " "));
        if (!$rows)
            return;
    ...
    function generate_image($source, $destination, $w, $h, $action_type = '', $filetype = '', $string = '', $jpeg_quality = '80') // —оздает изображение
    {
